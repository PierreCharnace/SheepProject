...
function createFolder () {
    ...
    if (!fs.existsSync('./imageAdmin')) {
        fs.mkdir('./imageAdmin', (err) => {
        if (err) {
        console.log(err)
        }
...
module.exports = multer({storage}).single('imageDescription'); // export multer who are configured